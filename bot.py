...
async def start_cmd(message: Message):
    ...
    if points is None:
        ...
        await message.answer(f"👋 خوش برگشتی\n💰 امتیاز: {points}\n/help")
    else:
...
async def points_cmd(message: Message):
    ...
    await message.answer(f"💰 امتیاز: {points}")
...
async def open_card(message: Message):
    ...
    await message.answer(f"📦 باقی مانده: {unopened_count(user_id)}")
